...
interface IRestructuringController {
    ...
    event RestructuringProposed(
        bytes32 indexed restructureId, bytes32 indexed loanId, bytes32 indexed snapshotId
    );
    event BorrowerConsentRecorded(
        bytes32 indexed restructureId, address indexed borrower, bytes32 consentDigest
    );
    event RestructuringVoteRecorded(
        bytes32 indexed restructureId,
        bytes32 indexed positionId,
        Phase9Types.VoteChoice choice,
        uint256 weight
    );
    event RestructuringExecuted(
        bytes32 indexed restructureId, bytes32 indexed executionEventId, bytes32 amendmentDigest
    );

    function propose(Phase9Types.RestructuringProposal calldata proposal)
        external
        returns (bytes32 restructureId);
    ...
    function execute(bytes32 restructureId, bytes32 operationId)
        external
        returns (Phase9Types.RestructuringExecutionResult memory result);
