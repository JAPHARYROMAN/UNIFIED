...
interface IInsuranceManager {
    ...
    event ClaimDecisionRecorded(
        bytes32 indexed decisionId, bytes32 indexed claimId, uint256 approvedAmount
    );
    event ClaimPaymentRecorded(
        bytes32 indexed claimPaymentId,
        bytes32 indexed claimId,
        address indexed beneficiary,
        uint256 amount
    );

    function registerPolicyVersion(Phase9ProtectionTypes.ReservePolicyVersion calldata policy_)
        external;
    ...
    function recordDecision(
        Phase9ProtectionTypes.ClaimDecision calldata decision,
        bytes[] calldata signatures
    ) external;
    function recordClaimPayment(Phase9ProtectionTypes.ClaimPayment calldata payment) external;
    function registerAdjudicatorSet(Phase9ProtectionTypes.AdjudicatorSet calldata adjudicatorSet)
        external;
