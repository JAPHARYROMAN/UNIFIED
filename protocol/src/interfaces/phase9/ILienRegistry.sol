...
interface ILienRegistry {
    ...
    event LienHandoffPending(
        bytes32 indexed collateralId, bytes32 indexed refinanceId, bytes32 indexed targetLoanId
    );
    event LienHandoffCompleted(
    ...
    ) external returns (bytes32 handoffId);
    function completeHandoff(bytes32 handoffId, bytes32 evidenceHash)
        external
        returns (Phase9Types.LienHandoffResult memory result);
