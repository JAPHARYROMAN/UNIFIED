...
interface ICollateralCustodyV2 {
    ...
    event CollateralCustodyRecorded(bytes32 indexed collateralId, bytes32 indexed assetId, address indexed borrower, uint256 quantity);
    event CollateralCustodyUpdated(bytes32 indexed collateralId, uint256 quantity, Phase9Types.CustodyStatus status, bytes32 operationId);

    function recordCustody(Phase9Types.CustodyRecord calldata record, bytes32 operationId) external;
    function updateCustody(bytes32 collateralId, uint256 quantity, Phase9Types.CustodyStatus status, bytes32 operationId) external;
