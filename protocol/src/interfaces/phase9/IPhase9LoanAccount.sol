...
interface IPhase9LoanAccount {
    ...
    event RefinancePayoffRecorded(bytes32 indexed refinanceId, uint256 amount, uint64 debtStateVersion);
    event ReplacementLoanActivated(bytes32 indexed refinanceId, uint64 debtStateVersion);
    event RestructuringApplied(bytes32 indexed restructureId, uint64 termsVersion, uint64 debtStateVersion);
    event LossRecorded(bytes32 indexed lossId, uint256 coveredLossExposure, uint64 debtStateVersion);
    event WriteOffRecorded(bytes32 indexed writeoffId, uint256 amount, uint64 debtStateVersion);
    event PostWriteOffRecoveryRecorded(bytes32 indexed recoverySourceId, uint256 amount, uint64 debtStateVersion);
    ...
    function recordRefinancePayoff(bytes32 refinanceId, uint256 amount, bytes32 operationId) external;
    function activateReplacementLoan(bytes32 refinanceId, Phase9Types.DebtState calldata initialDebt, bytes32 operationId) external;
    function applyRestructuring(Phase9Types.LoanAmendment calldata amendment, bytes32 operationId) external;
    ...
    function recordPostWriteOffRecovery(bytes32 recoverySourceId, uint256 amount, bytes32 operationId) external;
