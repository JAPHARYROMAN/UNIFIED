...
interface IReservePolicy {
    ...
    event ReservePolicyRegistered(bytes32 indexed policyVersionId, bytes32 indexed poolId, bytes32 indexed contentHash, uint64 effectiveAt);
    ...
    function restrictPolicy(bytes32 policyVersionId, bytes32 reasonCode, bytes32 operationId) external;
    function policy(bytes32 policyVersionId) external view returns (Phase9ProtectionTypes.ReservePolicyVersion memory);
