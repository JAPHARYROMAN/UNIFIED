...
interface IGuaranteeVault {
    ...
    event GuaranteeRegistered(bytes32 indexed guaranteeId, bytes32 indexed loanId, address indexed guarantor, uint256 maximumAmount);
    event GuaranteeReceiptRecorded(bytes32 indexed guaranteeId, bytes32 indexed receiptId, uint256 amount, uint256 paidAmount);

    function registerGuarantee(Phase9RecoveryTypes.Guarantee calldata guarantee_) external;
    function recordGuaranteeReceipt(bytes32 guaranteeId, bytes32 receiptId, uint256 amount, bytes32 balanceDeltaHash) external;
    ...
    function guarantee(bytes32 guaranteeId) external view returns (Phase9RecoveryTypes.Guarantee memory);
