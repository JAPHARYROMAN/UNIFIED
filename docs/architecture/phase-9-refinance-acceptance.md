...
interface, method activation, or candidate partition and does not open D3.
...
| `P9R-CHECK-002` | Attempt to accept only one refinance backlog item, only part of the method bundle, or evidence omitting an ADR 0022, ADR 0023, ADR 0024, or ADR 0025 pin | Activation fails unless `UNI-ADR-018`, `UNI-ADR-019`, `UNI-ADR-020`, `UNI-REFI-001`, and `UNI-REFI-002` are bound to the same reviewed head and every required method/evidence row and D1-D4 decision check passes; accepted specification, architecture, and topology controls alone activate nothing | ADR 0021 section 17, ADR 0022 section 6, ADR 0023 activation gate, ADR 0024 section 6, and ADR 0025 sections 1 and 6 |
...
| `P9R-EXEC-003` | Trace the success call, evidence, persistence, and log order | Operation/provisional-guard validation; canonical revalidation plus one-block snapshot, one representable `executed_at`, and balance baselines; quote consumption; payout legs 0/1; old payoff; begin-all, verify-all-pending, complete-all, verify-all-active lien calls; replacement activation; payout legs 2/3; commitment/escrow clearing; recomputed snapshots plus leg/unique-address/coordinator proofs; one completion write; frozen execution log; and one direct completion-transition log occur in that exact order in one transaction | ADR 0021 section 14 and ADR 0025 sections 2 through 5 |
...
| `P9R-EXEC-008` | Inject a revert at every dependency call and external token transfer, including registry mark failure, false terminal postcondition, wrong registered account, registry reentry, each of the four lien phases, snapshot recomputation, and aggregate delta proof | Registry terminality, debt, quote, payouts, liens, claims, escrow, versions, attempts, nonces, counters, results, and events revert to the exact prior `FUNDING_ESCROWED` state | ADR 0021 sections 14 and 15 and ADR 0025 sections 2 through 5 |
| `P9R-EXEC-009` | Retry exact and changed execute operation IDs after terminal lock release using the stored quote's pre-payoff debt version, the live post-payoff debt version, and forged/provisional state | Exact replay recomputes the ID only from the consumed quote's stored pre-payoff version, requires processed operation plus `COMPLETED`/attempt-one/matching nonzero terminal result, branches before lock ownership, and returns without a second effect/event; current post-payoff version, provisional `EXECUTING`, and changed reuse cannot replay | ADR 0021 section 16 and ADR 0025 section 4 |
...
| `P9R-FZ-001` | Run ABI/storage/compiler/source-set compatibility, linked-module, and generated-freshness checks, including constructor-less initializer locking and OZ-compatible private clone helpers | Historical freeze remains intact and ADR 0025 changes evidence semantics only; only the reviewed event, two errors, activated method bodies, exact three same-file libraries, seven compiler-linked coordinator call sites, and two AST-pinned assembly exceptions are accepted; no constructor ABI item, imported clone error, selector, interface, tuple/storage field, base, logical slot, offset, type, module storage, nested link/delegatecall, compiler-setting, unresolved-link, or EIP-170 drift exists, and historical `Phase9ImplementationNotFrozen` remains present | ADR 0021 sections 3 and 17, ADR 0022 sections 1, 3, and 6, ADR 0023 sections 1 through 8, and ADR 0025 sections 1 and 6 |
