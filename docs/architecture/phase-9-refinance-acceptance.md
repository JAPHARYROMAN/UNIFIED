...
| `P9R-BOOT-005` | Recalculate new-loan/bootstrap/creation/clone/activation/tranche/position/custody/lien identities independently, including operation-ID-bound custody identity and equal refinance/new-loan nonce, then replay factory/setup calls | Every preimage is acyclic; only custody's passed operation ID is contract-authoritative while activation/tranche/position/lien operation hashes are correlation evidence; factory/setup exact replay is inert or returns stored clones, changed reuse conflicts, and exact request replay rejects before a quote | Refinance reference evidence |
...
| `P9R-ID-003` | Submit nonzero caller refinance/quote/request digest/derived state; unequal/zero/high-bit/exhausted refinance/new-loan nonces; concurrent same/next nonce requests; then repeat success | Derived or unequal nonce input fails before effects; the high-bit lock permits one owner and one shared refinance/new-loan nonce, while concurrent/exact repeat fails before another quote and a later released nonce gets a distinct replacement/quote | ADR 0021 sections 4, 6, 11, and 16 |
