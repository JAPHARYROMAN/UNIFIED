...
refinance slice governed by ADR 0019, ADR 0020, ADR 0021, and ADR 0022. ADR 0021
...
interpreted by ADR 0022, and only after every applicable `P9R-*` row below passes. A
passing document review is not implementation evidence.
...
| `P9R-COMPAT-003` | Scan the candidate source set for added selectors, other events/errors, tuple/storage fields, bases, constructor ABI items, imported clone errors, and layout drift | No unapproved surface or storage change exists; directly deployed account/manager implementations are locked through their existing final initialized flags and the factory retains the historical freeze error only through an unreachable compatibility marker | ADR 0021 section 3 and ADR 0022 sections 1, 3, and 6 |
| `P9R-CHECK-001` | Evaluate the method-level activation manifest against the exact source head | Only ADR-activated methods may differ from their fail-closed bodies; every unopened mutator retains the exact freeze error | ADR 0021 section 17 and ADR 0022 section 6 |
| `P9R-CHECK-002` | Attempt to accept only one refinance backlog item, only part of the method bundle, or evidence omitting an ADR 0022 pin | Activation fails unless `UNI-REFI-001` and `UNI-REFI-002` are bundled and every required method/evidence row and D1 decision check passes | ADR 0021 section 17 and ADR 0022 section 6 |
...
| `P9R-DEPLOY-003` | Verify every manifest transaction, receipt/log, code, constructor argument, runtime hash, dependency getter, role expiry/membership, reviewed storage slot, and direct account/manager implementation lock | Candidate and post-broadcast evidence agree exactly through canonical block-hash reads; implementations reject initialization while factory-created clones initialize once; missing, stale, or mismatched evidence fails | Refinance deployment evidence and ADR 0022 section 3 |
...
| `P9R-BOOT-005` | Recalculate new-loan/bootstrap/creation/clone/activation/tranche/position/custody/lien identities independently, including operation-ID-bound custody identity and equal refinance/new-loan nonce; replay bootstrap creation after later successful creations and after the bootstrap loan becomes terminal; mutate the stored request/resolver facts; and reuse the loan under another creation ID | Every preimage is acyclic; only custody's passed operation ID is contract-authoritative while activation/tranche/position/lien operation hashes are correlation evidence; exact factory replay branches before the current nonce, verifies canonical registry identity regardless of the current terminal flag, and returns its verified original clones without effects; changed processed creation facts revert `InvalidPhase9LoanConfiguration`; an alternate creation collision reverts `Phase9LoanAlreadyExists`; exact setup replay is inert; and exact outer request replay rejects before a quote | Refinance reference evidence and ADR 0022 section 2 |
...
| `P9R-AUTH-008` | Scan for delegation, signature, arbitrary-call, rescue, sweep, recipient-selection, operator override, or caller-authored factory/account/manager authority paths | No such authority is reachable; the coordinator comes from the active creation/account configuration, account initialization is factory-only, and manager initialization authenticates that factory through the already initialized account | ADR 0021 sections 1, 5, and 11 and ADR 0022 section 3 |
...
| `P9R-FZ-001` | Run ABI/storage/compiler/source-set compatibility and generated-freshness checks, including constructor-less initializer locking and OZ-compatible private clone helpers | Historical freeze remains intact and only the reviewed event, two errors, and activated method bodies are accepted; no constructor ABI item, imported clone error, selector, tuple/storage field, base, slot, offset, or type drift exists, and historical `Phase9ImplementationNotFrozen` remains present | ADR 0021 sections 3 and 17 and ADR 0022 sections 1, 3, and 6 |
