...
| `P9R-EXEC-003` | Trace the success call, evidence, persistence, and log order | After a storage-only terminal-replay classification misses, first execution reads the stored quote, recomputes the operation ID from its pre-payoff version, requires supplied-ID equality, then enters the provisional guard; canonical revalidation plus one-block snapshot, one representable `executed_at`, and balance baselines; quote consumption; payout legs 0/1; old payoff; begin-all, verify-all-pending, complete-all, verify-all-active; replacement activation; payout legs 2/3; escrow clearing; recomputed proofs; one completion write; frozen execution log; and one direct completion-transition log occur in that exact order | ADR 0021 section 14 and ADR 0025 sections 2 through 5 |
