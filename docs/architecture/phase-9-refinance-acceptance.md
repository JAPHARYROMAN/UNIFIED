...
freeze in ADR 0025 and repartitioned by ADR 0026. ADR 0021
...
ADR 0026 replaces its operative module count, execution ownership, linked-call count,
and deployment order. ADR 0024 selects explicit Anvil nonce preconditioning and the exact
...
interface, or method activation and does not open D3. ADR 0026 fixes five same-file
libraries, an exact two-call execution protocol, an all-static 68-word/2,176-byte
execution plan, eight compiler links, and a twelve-CREATE nonce-12 coordinator target
while implementation and production topology migration remain closed.
...
interpreted by ADR 0022 and partitioned by ADR 0026, and only after every applicable
...
deployment evidence. ADR-0026 acceptance requires a newly generated twelve-CREATE graph;
the existing ten-CREATE script, schema, plan, and observations cannot be relabeled.
...
- The frozen coordinator surface consists only of `requestRefinance`,
  ...
  `refundCommitment`.
- The ADR-0026 target source contains exactly the same protocol-facing coordinator plus
  the same-file `Phase9RefinanceValidationModule`, `Phase9RefinanceRequestModule`,
  `Phase9RefinanceLifecycleModule`, `Phase9RefinanceExecutionPrepareModule`, and
  `Phase9RefinanceExecutionFinalizeModule`. The coordinator has exactly eight fixed
  compiler-linked call sites: request `begin`, validation `preflight`, request
  `complete`, lifecycle funding/cancel/refund, execution prepare, and execution
  finalize. No
  module may link or delegate again, and none of these library entries is protocol ABI.
- First execution transports only the exact all-static `ExecutionPlanV1`: 68 ABI words,
  exactly 2,176 bytes, fixed arrays with zero tails, normative ADR 0025 hashes, and a
  separately returned `planHash`. The coordinator checks exact length/header/suffix/hash,
  provisional guard, active lock, full escrow, zero attempt/evidence, and unprocessed
  operation, then forwards identical bytes without a catch or intervening external call.
  Replay returns before finalization and before every dependency read.
...
| `P9R-COMPAT-003` | Scan the candidate source set for added selectors, other events/errors, tuple/storage fields, bases, constructor ABI items, imported clone errors, layout drift, undeclared libraries, module storage, nested links, and delegatecall drift | No unapproved surface or storage change exists; exactly the five ADR 0026 same-file libraries are present and storage-free; the validation module is storage-blind; only the request, lifecycle, and execution modules use the exact slot-zero mirror; directly deployed account/manager implementations remain locked; and the historical freeze error remains only through its approved compatibility marker | ADR 0021 section 3, ADR 0022 sections 1, 3, and 6, ADR 0023 section 3, and ADR 0026 sections 1 and 8 |
| `P9R-CHECK-001` | Evaluate the method-level activation manifest and linked-module checker against the exact source head | Only ADR-activated methods may differ from their fail-closed bodies; every unopened mutator retains the exact freeze error; the exact five-module/eight-call compiler-linked candidate passes its pinned plan, layout, assembly, opcode, link, ABI, compiler, 22,118-byte execution-module budget, EIP-170, and EIP-3860 gates | ADR 0021 section 17, ADR 0022 section 6, and ADR 0026 sections 1, 2, 4, 6, and 8 |
| `P9R-CHECK-002` | Attempt to accept only one refinance backlog item, only part of the method bundle, or evidence omitting an ADR 0022 through ADR 0026 pin | Activation fails unless the existing prerequisite backlog rows, both refinance implementation rows, ADR 0026 repartition, and every required method/evidence row and D1-D4 decision check bind the same reviewed head; accepted specification, architecture, and topology controls alone activate nothing | ADR 0021 section 17, ADR 0022 section 6, ADR 0024 section 6, ADR 0025 sections 1 and 6, and ADR 0026 section 9 |
...
| ID | Test | Required result | Trace |
|---|---|---|---|
| `P9R-DEPLOY-001` | Deploy the full refinance graph using ADR 0024's explicit nonce-preconditioning and role-initialization order with ADR 0026's graph | A pairwise-distinct roleless candidate broadcaster is observed at nonce zero, preconditioned exactly once to nonce one, and sends exactly twelve top-level `CREATE`s at nonces 1 through 12 in ADR 0026 order; the complete role-absent graph verifies before one distinct governance executor grants the factory role | ADR 0024 sections 1, 2, 4, and 5 and ADR 0026 section 7 |
| `P9R-DEPLOY-002` | Independently verify the prerequisite RoleManager, nonce precondition, nonce-12 coordinator, nonce-6-through-10 modules, all eight fixed links, reciprocal constructor bindings, and final governance role initialization | The setup administrator and governance executor receive only their constructor roles; the candidate receives none; the coordinator prediction uses RLP nonce byte `0x0c` and equals `0xca03dc4665a8c3603cb4fd5ce71af9649dc00d44`; latest and pending candidate nonces end at `0x0d`; the factory role remains absent through topology verification; then the governance executor's only nonce-zero transaction grants `LOAN_FACTORY_ROLE` to the verified factory with maximum expiry | ADR 0024 sections 2, 4, and 5 and ADR 0026 section 7 |
| `P9R-DEPLOY-003` | Verify every activation-grade manifest transaction, receipt/log, code, constructor argument, module self-address runtime patch, link offset, fully linked runtime hash, dependency getter, role expiry/membership, reviewed storage slot, runtime/initcode size, execution-module budget, and direct account/manager implementation lock | Candidate and post-broadcast evidence agree exactly through canonical block-hash reads; all five modules and the coordinator match pinned compiler output and applicable budgets; the exact single role grant is last; missing, stale, unresolved, swapped, extra, or mismatched evidence fails | Refinance deployment evidence, ADR 0022 section 3, ADR 0024 sections 4 through 7, and ADR 0026 sections 6 through 8 |
| `P9R-DEPLOY-004` | Attempt public-chain, reused/production key, real value, nonce-zero candidate RoleManager, wrong or unevidenced nonce preconditioning, CREATE2 top-level, reordered/undeclared transaction, premature or wrong-sender role grant, role-admin change, setup/business action, post-hoc repair, or external-provider deployment | Every attempt is rejected, requires bounded reset, and creates no accepted deployment checkpoint | ADR 0021 sections 1 and 18, ADR 0024 section 7, and ADR 0026 sections 7 through 9 |
...
| `P9R-AUTH-008` | Scan for caller-selected delegation, nested module links/delegatecalls, signature, arbitrary-call, rescue, sweep, recipient selection, operator override, caller-authored execution plans, or caller-authored factory/account/manager authority paths; exercise zero/coordinator, aliased, settlement-token, and other system-contract canonical recipients | No caller-selected authority is reachable; only ADR 0026's exact eight coordinator-to-fixed-library compiler links exist; no module delegates again; execution plan bytes are prepare-produced, exact-static, coordinator-checked, never external or persisted, and forwarded unchanged; zero/coordinator/settlement-token recipients fail before effects; and canonical dependency identity remains unchanged | ADR 0021 sections 1, 5, and 11, ADR 0022 section 3, ADR 0025 section 3, and ADR 0026 sections 2, 4, and 8 |
...
| `P9R-EXEC-003` | Trace the success call, exact prepare/finalize transport, evidence, persistence, and log order | Storage-only replay returns before dependencies and finalization. First execution enters the provisional guard, completes quote consumption, payout legs 0/1, payoff, and midpoint proofs in prepare; the coordinator validates the exact 68-word/2,176-byte static plan and returned hash and forwards identical bytes without catch or intervening external call; finalize repeats every check and re-resolution before the uninterrupted lien barrier, replacement activation, payout legs 2/3, conservation, escrow clearing, final proofs, completion write, frozen execution log, and direct completion-transition log | ADR 0021 section 14, ADR 0025 sections 2 through 5, and ADR 0026 sections 3 through 5 |
...
| `P9R-FZ-001` | Run ABI/storage/compiler/source-set compatibility, linked-module, execution-plan, and generated-freshness checks, including constructor-less initializer locking and OZ-compatible private clone helpers | Historical freeze remains intact; only the reviewed ABI additions, activated bodies, exact five same-file libraries, eight compiler-linked coordinator call sites, exact 68-word/2,176-byte plan, and reviewed assembly exceptions are accepted; no constructor ABI item, selector, interface, tuple/storage field, base, logical slot, offset, type, module storage, nested link/delegatecall, compiler-setting, unresolved-link, execution-module budget, EIP-170, or EIP-3860 drift exists | ADR 0021 sections 3 and 17, ADR 0022 sections 1, 3, and 6, ADR 0025, and ADR 0026 sections 1, 2, 4, 6, and 8 |
